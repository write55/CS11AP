...
public class Registration {
    ...
    public static String getInput() throws IOException {
        ...
        System.out.print("Enter ID and first letter of course (capitalized) separated by a space: ");
        String input = in.readLine();
        // int id = new Integer(input.substring(0, input.indexOf(" ") - 1));
        char course = Character.toUpperCase(input.charAt(input.length() - 1));
        while (course != 'B' && course != 'C' && course != 'P' && course != 'S' && course != 'W') {
            ...
            input = in.readLine();
            course = Character.toUpperCase(input.charAt(input.length() - 1));
        }
        return input;
    }

    public static String getCourseName(char c) {
        String name;
        switch (c) {
            case 'B':
                name = "Batik";
